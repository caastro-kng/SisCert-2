...
const COURSE_NAMES = [
  CVTE_COURSE,
  'Curso Especializado para Condutores de Veículos de Transporte de Produto Perigoso',
  'Curso Especializado para Condutores de Transporte Coletivo de Passageiros',
  'Curso Especializado para Condutores de Veículos de Transporte de Carga Indivisível',
];
...
export const EmitirCertificadoPage: React.FC<EmitirCertificadoPageProps> = ({
  ...
  const [course, setCourse] = useState(CVTE_COURSE);
  ...
  useEffect(() => {
    ...
    if (editingCertificate) {
      ...
      setCourse(editingCertificate.course || CVTE_COURSE);
    ...
    if (reissuingFrom) {
      ...
      setCourse(reissuingFrom.course || CVTE_COURSE);
  ...
  return (
    <div id="emitir-certificado-page" className="space-y-8">
      ...
      {currentStep === 1 && <section className="space-y-3 rounded-lg border border-gray-200 bg-white p-4 shadow-sm"><div><h3 className="text-sm font-bold text-gray-900">Dados das disciplinas</h3><p className="text-[11px] text-gray-500">As disciplinas seguem o modelo institucional; edite apenas carga horária, avaliação e instrutor.</p></div>{subjectDetails.map((subject, index) => <div key={subject.name} className="grid grid-cols-1 gap-2 rounded-md border border-gray-200 p-3 sm:grid-cols-[1.5fr_.7fr_.8fr_1.2fr]"><p className="self-center text-xs font-bold text-[#1B4332]">{subject.name}</p><input value={subject.workload} onChange={(e) => setSubjectDetails((items) => items.map((item, itemIndex) => itemIndex === index ? { ...item, workload: e.target.value } : item))} className={inputClass(`subject-workload-${index}`)} placeholder="Carga h/a" /><input value={subject.evaluation} onChange={(e) => setSubjectDetails((items) => items.map((item, itemIndex) => itemIndex === index ? { ...item, evaluation: e.target.value } : item))} className={inputClass(`subject-grade-${index}`)} placeholder="Nota" /><input value={subject.instructorName} onChange={(e) => setSubjectDetails((items) => items.map((item, itemIndex) => itemIndex === index ? { ...item, instructorName: normalizeName(e.target.value) } : item))} className={inputClass(`subject-instructor-${index}`, 'uppercase')} placeholder="NOME DO INSTRUTOR" /></div>)}</section>}
